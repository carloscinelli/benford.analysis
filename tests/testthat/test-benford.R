...
test_that("Both signs, simulated log-normal and plots",
          {
            ...
            xyplot.Berford(bfd, obs.freq = "digits", freq = T)
            xyplot.Berford(bfd, obs.freq = "second order", freq = T)
            xyplot.Berford(bfd, obs.freq = "digits", freq = F)
            xyplot.Berford(bfd, obs.freq = "second order", freq = F)
            needle.Benford(bfd, discrepancy = "chi squared")
            rootogram.Benford(bfd, obs.freq = "digits")
            rootogram.Benford(bfd, obs.freq = "second order")
            rootogram.Benford(bfd, obs.freq = "digits", err.bounds = T)
            rootogram.Benford(bfd, obs.freq = "second order", err.bounds = T)
            rootogram.Benford(bfd, obs.freq = "digits", freq = F)
            rootogram.Benford(bfd, obs.freq = "second order", freq = F)
            histogram.Benford(bfd, obs.freq = "digits")
            histogram.Benford(bfd, obs.freq = "second order")
            histogram.Benford(bfd, obs.freq = "summation")
            histogram.Benford(bfd, obs.freq = "digits", err.bounds = T, freq = F)
            histogram.Benford(bfd, obs.freq = "second order", freq = F)
            histogram.Benford(bfd, obs.freq = "summation", freq = F)
